  ...
  methods: {
    async getListFn() {
      return await searchResultListAPI({
        keywords: this.value,
        limit: 20,
        offset: (this.page - 1) * 20, // 固定公式根据文档得来
    ...
    async inputFn() {
      ...
      this.resultList = res.data.result.songs;
      this.loading = false; //为了触发触底部事件加载更多

      },900)
      
    },
    async onLoad(){//触底事件 ,要加载下一页的和数据内部会自动把loading改为true
      this.page++
      const res = await this.getListFn()
      if(res.data.result.songs === undefined){// 没有更多数据了
        this.finished = true //全部加载完成(list不会在触发onload方法)
        this.loading = false //本次加载完成 , 保证下一次还能触发onload
        return
      }
      this.resultList = [...this.resultList, ...res.data.result.songs] //数据拼接  ...args为剩余参数
      this.loading = false //保证下一次还能触发onload方法
